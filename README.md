...
- 支持扫码核销预约二维码。
- 支持查询二维码状态。
- 支持从“我的预约（待签到）”一键跳转校园导航并自动带入目标地点。
...
- 支持按教学楼查看课表（按日/周/月维度）。
- 支持教室课表查询与时段占用状态查询。
- 支持按楼层、教室关键字、课程关键字、时间段筛选教学楼日课表。
- 支持“我的课程”按日/按周查看。
- 支持课程星标、课程备注、课前提醒时间配置。
...
- 支持校园地图浏览（教学楼 + 公共场馆点位）。
- 支持按名称、编号、地址搜索目标点位。
- 支持定位“我的位置”并展示与目标点位距离。
- 支持一键规划步行路线（路径服务不可用时自动降级为直线导航）。
- 支持在预约场景中携带目标名称自动选中并尝试规划路线。
...
- `/building-schedule/:buildingId`：教学楼课表
- `/reservation`：预约中心
...
- `/profile/my-courses`：我的课程
- `/profile/teams`：我的协作
- `/profile/teams/:teamId/chat`：小组聊天
- `/profile/team-messages`：协作消息中心
- `/profile/plans`：我的计划
